...
import crypto from "crypto"; // FIXED: Added missing crypto package import
...
export const sendOTP = async (req, res, next) => {
  try {
    ...
    const otp_expiry_time = Date.now() + 10 * 60 * 1000; // 10 minutes validation window

    // 2. Locate the user profile document by email and save the new parameters
    ...
    // 3. Connect to your custom mail transport provider instance context
    const transporter = nodemailer.createTransport({
      service: 'Gmail',
      auth: {
        user: ENV.EMAIL_USER,
...
export const verifyOTP = async (req, res, next) => {
  try {
    ...
    if (!email || !otp) {
        return res.status(400).json({ success: false, message: "Email and OTP are both required fields." });
    }
    ...
    // Clear verification fields safely out of database space upon success
    user.verified = true;
    user.otp = undefined;
    ...
    return res.status(200).json({
      success: true, // Standardized object keys formatting parameter layout
